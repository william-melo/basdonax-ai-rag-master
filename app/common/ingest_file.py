...
# Añadir esta nueva función después de la función ingest_file existente

def ingest_files(uploaded_files):
    if does_vectorstore_exist(CHROMA_SETTINGS):
        db = Chroma(embedding_function=embeddings, client=CHROMA_SETTINGS)
        for uploaded_file in uploaded_files:
            texts = process_file(uploaded_file, uploaded_file.name)
            if texts is None:
                st.warning(f'El archivo {uploaded_file.name} ya fue agregado anteriormente.')
            else:
                st.spinner(f"Creando embeddings para {uploaded_file.name}.")
                db.add_documents(texts)
        st.success(f"Se agregaron los archivos con éxito.")
    else:
        st.error("No se pudo crear o acceder al vectorstore.")
...
def delete_files_from_vectordb(filenames: List[str]):
    ...
    if errors:
        print("Ocurrieron los siguientes errores:")
        for error in errors:
            print(error)
    
    return successful_deletions, errors